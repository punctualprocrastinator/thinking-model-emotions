...
MAX_NEW_TOK  = 4096               # Hard cap — 4096 lets model finish thinking on hard GPQA
...
def _build_prompt(tokenizer, problem: str) -> str:
    """
    Apply OLMo-3 chat template. We DO NOT override the system prompt,
    because RLHF thinking models often break their formatting (like emitting </think>)
    if their default system prompt is overridden.
    We append our format instructions to the user message instead.
    """
    messages = [
        {
            "role": "user", 
            "content": (
                f"{problem.strip()}\n\n"
                f"Please think concisely. You must conclude your thinking by writing exactly '</think>' "
                f"before outputting your final answer cleanly, preferably inside \\boxed{{}}."
            )
        },
    ]
...
def _think_boundary(token_ids: list, tokenizer) -> tuple:
    ...
    Optimised: single full-text decode + string search first (O(1) decode calls).
    Only does precise token-level search when </think> IS found.
    """
    TARGET = "</think>"
    # Fast path: one decode of full text, check if </think> exists at all
    ...
    # </think> found — do precise token-level boundary via sliding window
    # Only search in a narrow region around the estimated token position
    # Estimate token position from char position (avg ~3-4 chars/token)
    est_tok = max(0, int(char_pos / 4) - 20)
    WINDOW = 6
    for i in range(est_tok, len(token_ids)):
    ...
    # Fallback: character-counting approach
    char_count = 0
...
class ThinkEndCriteria(StoppingCriteria):
    """Stop generation the moment the string </think> appears in ALL sequences in the batch."""
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor,
        ...
        if new_toks.shape[1] < 3:
            return False
            
        if self.finished is None or len(self.finished) != input_ids.shape[0]:
            self.finished = [False] * input_ids.shape[0]
        
        # Decode the last 20 tokens for each sequence
        tail_ids = new_toks[:, -20:].tolist()
        for i, seq_ids in enumerate(tail_ids):
...
def capture_batch(
    ...
    For each problem × n_samples:
      Pass 1 — generate thinking (batched across n_samples), stopping at </think>.
      Pass 2 — short generation for the answer (per sample, only when </think> found).
      Pass 3 — BATCHED forward pass on all n_samples simultaneously to capture
               residual stream activations. All samples are padded to the same
               length and processed in a single model call for ~5× speedup.

    Includes incremental checkpointing every 10 problems.
    """
    import gc
    ...
    start_idx = 0
    if checkpoint_tag:
        inc_path = CACHE_DIR / f"{checkpoint_tag}_incremental.pkl"
        if inc_path.exists():
            with open(inc_path, "rb") as f:
                saved = pickle.load(f)
            all_results = saved["caps"]
            start_idx = len(all_results)
            log.info(f"  Resuming from incremental checkpoint: {start_idx}/{len(problems)} done")

    for prob_idx, prob in enumerate(tqdm(problems, desc="Capturing", initial=start_idx)):
        ...
        # Batch identical prompts to parallelize n_samples
        input_ids    = inputs.input_ids.repeat(n_samples, 1)
        attn_mask    = inputs.attention_mask.repeat(n_samples, 1)
        prompt_len   = input_ids.shape[1]

        try:
            # ── Pass 1: generate thinking (batched) ───────────
            stopping = StoppingCriteriaList([ThinkEndCriteria(tokenizer, prompt_len)])
            think_kwargs = dict(
            ...
            all_results.append([None] * n_samples)
            continue

        # ── Pre-process all samples: boundaries, text, Pass 2 ─────────────
        sample_meta = []  # list of dicts or None per sample
        for i in range(n_samples):
            try:
                think_seq = think_out[i]
                think_new = think_seq[prompt_len:].tolist()

                think_end, answer_start = _think_boundary(think_new, tokenizer)

                # Track whether </think> was genuinely found vs. hit token cap
                think_found = (think_end < len(think_new))

                # Apply 1500-token cap BEFORE expensive decoding when </think> missing
                if not think_found:
                    log.warning(f"  [{prob.get('id','?')}] </think> not found in {len(think_new)} tokens")
                    think_end = min(think_end, 1500)
                ...
                # ── Pass 2: generate answer ONLY when </think> was genuinely found ──
                answer_text = ""
                if think_found:
                    seed_ids = think_seq[:prompt_len + answer_start].unsqueeze(0)
                    seed_mask = torch.ones_like(seed_ids)
                    with torch.no_grad():
                        ans_out = model.generate(
                            seed_ids, max_new_tokens=512, do_sample=False,
                            pad_token_id=tokenizer.eos_token_id,
                            attention_mask=seed_mask,
                        )
                    ans_new = ans_out[0][seed_ids.shape[1]:].tolist()
                ...
                sample_meta.append({
                    ...
                    "think_tokens": think_ids,  # store raw IDs, decode lazily
                    "answer_text":  answer_text,
                })
            except Exception as e:
                log.warning(f"  Sample {i} failed (pre-process): {e}")
                sample_meta.append(None)

        # ── Pass 3: BATCHED forward pass for activations ──────────────────
        # Collect valid samples and pad to same length for batching
        valid_indices = [i for i, m in enumerate(sample_meta) if m is not None]
        ...
        if valid_indices:
            ...
            for i in valid_indices:
                m = sample_meta[i]
                seq = m["think_seq"][:prompt_len + m["think_end"]]
                seqs.append(seq)
                seq_lens.append(seq.shape[0])

            max_len = max(seq_lens)
            batch_ids = torch.full((len(valid_indices), max_len),
                                   tokenizer.pad_token_id,
                                   dtype=seqs[0].dtype, device=device)
            batch_mask = torch.zeros(len(valid_indices), max_len,
                                     dtype=torch.long, device=device)
            for j, seq in enumerate(seqs):
                batch_ids[j, :seq.shape[0]] = seq
                batch_mask[j, :seq.shape[0]] = 1
            ...
            # Extract per-sample activations from the batched output
            for j, i in enumerate(valid_indices):
                m = sample_meta[i]
                n_think = len(m["think_ids"])
                acts_per_layer = []
                pre_think_per_layer = []
                for li in layers:
                    rs = captured[li]  # (batch, seq, d_model)
                    acts_per_layer.append(
                        rs[j, prompt_len: prompt_len + n_think, :].numpy()
                    )
                    pre_think_per_layer.append(
                        rs[j, prompt_len - 1, :].numpy()
                    )

                samples[i] = {
                    "think_acts":    np.stack(acts_per_layer),
                    "pre_think_act": np.stack(pre_think_per_layer),
                    "think_tokens":  [tokenizer.decode([t]) for t in m["think_tokens"]],
                    "think_text":    m["think_text"],
                    "answer_text":   m["answer_text"],
                }

            del captured, batch_ids, batch_mask
        # ── end Pass 3 ────────────────────────────────────────────────────
        ...
        # ── Incremental checkpoint every 10 problems ──────────────────────
        if checkpoint_tag and (len(all_results) % 10 == 0):
            inc_path = CACHE_DIR / f"{checkpoint_tag}_incremental.pkl"
            with open(inc_path, "wb") as f:
                pickle.dump({"caps": all_results}, f, protocol=pickle.HIGHEST_PROTOCOL)
            log.info(f"  Incremental checkpoint: {len(all_results)}/{len(problems)}")

    # Clean up incremental checkpoint after completion
    if checkpoint_tag:
        inc_path = CACHE_DIR / f"{checkpoint_tag}_incremental.pkl"
        if inc_path.exists():
            inc_path.unlink()
...
if __name__ == "__main__":
    ...
    if TEST_MODE:
        ...
        # Sized for 6-hour budget on GH200 with MAX_NEW_TOK=4096.
        # 100 primary problems × 6 samples = 600 traces → ~100+ contrastive pairs
        N_GPQA       = 60     # GPQA Diamond — grad-level science, acc ~45-55%
        N_MMLU       = 40     # MMLU hard subjects — acc ~50-70%
        USE_AIME     = False  # AIME skipped; GPQA provides plenty of difficulty
        N_MGSM       = 8      # MGSM per language (6 non-Latin langs) — cross-lang transfer
        N_SAMPLES    = 6      # 6 samples to guarantee contrastive pairs
